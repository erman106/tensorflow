...
class HostKernel : public Kernel {
 public:
  explicit HostKernel(HostExecutor* host_executor)
  ...
  void SetExecutionEngine(
      std::unique_ptr<LlvmExecutionEngine> execution_engine) {
    execution_engine_ = std::move(execution_engine);
  }

  void SetExecutionEngine(
      std::unique_ptr<CppExecutionEngine> execution_engine) {
    execution_engine_ = std::move(execution_engine);
  }

 private:
  HostExecutor* host_executor_ = nullptr;
