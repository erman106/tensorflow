...
absl::Status HostExecutor::GetKernel(const MultiKernelLoaderSpec& spec,
  ...
  if (spec.has_llvm_host_kernel()) {
    ...
    if (!execution_engine.ok()) {
      return execution_engine.status();
    }
    host_kernel->SetExecutionEngine(std::move(execution_engine.value()));
