...
/*static*/ RuntimeExecutionEngine::SymbolsBinding
RuntimeExecutionEngine::BindAll(std::vector<SymbolsBinding> bindings) {
...
RuntimeExecutionEngine::CreateFromModule(
  ...
  //         StrFormat("failed to set up exported function %s interface: %s",
  //         name,
  //                   status.message()));
  //   }
  // }
  ...
  if (options.symbols_binding) {
    ...
    if (auto err = main_jd.define(symbols))
      return InternalError(
          StrFormat("failed to add symbols bindings: %s", ToString(err)));
  }
