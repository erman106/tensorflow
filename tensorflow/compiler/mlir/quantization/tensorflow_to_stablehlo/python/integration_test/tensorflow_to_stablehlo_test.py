...
def build_tensorflow_mlir(tempdir) -> str:
  assembly = """
    ...
    """
  filename = tempdir + '/add_one_model.mlir'
  ...
  def test_saved_model_to_stablehlo(self):
    with tempfile.TemporaryDirectory() as tempdir:
      ...
      with ir.Context() as ctx:
        ...
        assert (
            re.search(r'stablehlo.add.*: tensor<4xf32>', str(module))
            is not None
        )

  def test_tf_mlir_to_stablehlo(self):
    with tempfile.TemporaryDirectory() as tempdir:
      path = build_tensorflow_mlir(tempdir)
      module_bytecode = tensorflow_to_stablehlo.tensorflow_mlir_to_stablehlo(
          input_path=path,
