...
class InsertWeightParamPattern
 ...
 public:
  ...
  LogicalResult match(Operation* op) const override {
    ...
    return success(op->hasOneUse() &&
                   IsWeightQuantizableFunction(*op->getUses().begin(), rank));
  }
  ...
  static bool IsWeightQuantizableFunction(OpOperand& operand, int64_t rank) {
    ...
    return HasValidWeightOnlyPtqMethod(weight_only_ptq, rank);
  }
  ...
  void rewrite(Operation* op, PatternRewriter& rewriter) const override {
    ...
    if (IsPerTensor(weight_only_ptq)) {
      ...
      auto xla_call_module_op = cast<TF::XlaCallModuleOp>(quantizable_op);
      int quantization_dimension =
          GetQuantizationDimension(weight_only_ptq, xla_call_module_op);
 ...
 private:
  static bool HasValidWeightOnlyPtqMethod(const WeightOnlyPtq& weight_only_ptq,
                                          int64_t rank) {
    if (weight_only_ptq.input_quantized_types().empty()) {
      return true;
    }
    if (!weight_only_ptq.input_quantized_types().contains(1)) {
      return false;
    }
    ...
    if (quantized_type.has_dimension_specs() &&
      ...
      int quantization_dimension = quantized_type.dimension_specs().dimension();
      return quantization_dimension >= 0 && quantization_dimension < rank;
    }
    return true;
  }
  ...
  static bool IsPerTensor(const WeightOnlyPtq& weight_only_ptq) {
    ...
    const QuantizedType& quantized_type =
        weight_only_ptq.input_quantized_types().at(1);
    return quantized_type.has_per_tensor();
  }
